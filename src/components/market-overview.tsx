...
export function MarketOverview({ artist, range }: MarketOverviewProps) {
  ...
  const chartData = getHistory(artist, range)
    .map((point) => ({ date: point.date, value: Number((point.pulse / 10).toFixed(1)) }))
  const dateIndexes = [...new Set([0, Math.floor((chartData.length - 1) / 2), chartData.length - 1])]
  ...
  return <section>
    ...
    <div className="border border-border p-5 sm:p-6">
      <div className="flex items-start justify-between gap-4"><div><p className="text-sm font-medium">{artist.name} <span className="text-muted-foreground">type beat</span></p><div className="mt-2 flex flex-wrap gap-1.5"><p className="inline-flex border border-primary/30 bg-primary/[0.06] px-2 py-1 font-heading text-[9px] tracking-wide text-primary">{artist.hasData ? artist.status : artist.sampleSize === 0 ? "no data" : "collecting data"}</p>{artist.hasData && <p className={cn("inline-flex border px-2 py-1 font-heading text-[9px] tracking-wide", confidenceColor)}>{confidenceLabel}{artist.sampleSize !== undefined ? ` · ${artist.sampleSize} videos` : ""}</p>}</div><p className="mt-2 font-heading text-[9px] tracking-wide text-muted-foreground">{artist.hasData ? artist.reason : artist.sampleSize === 0 ? "no videos found in the latest sample" : "history starts after the first daily refresh"}</p></div><div className="text-right"><p key={artist.score} className="value-pop font-heading text-3xl tracking-[-0.08em]">{artist.hasData ? (artist.score / 10).toFixed(1) : "—"}</p><p className={cn("mt-1 font-heading text-[11px] tabular-nums", roundedChange !== null && roundedChange > 0 ? "text-primary" : roundedChange !== null && roundedChange < 0 ? "text-destructive" : "text-muted-foreground")}>{roundedChange === null ? "—" : `${roundedChange > 0 ? "+" : ""}${roundedChange}%`} / {range}</p></div></div>
      ...
      </> : <div className="flex h-44 items-center justify-center font-heading text-[10px] text-muted-foreground">{artist.hasData ? "history unavailable" : artist.sampleSize === 0 ? "no data" : "collecting data"}</div>}
      {artist.hasData && change === null && <p className="mt-3 font-heading text-[9px] text-muted-foreground">not enough history for a {parseInt(range)}-day change</p>}
