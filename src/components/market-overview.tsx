...
export function MarketOverview({ artist, range }: MarketOverviewProps) {
  ...
  return <section>
    ...
    <div className="border border-border p-5 sm:p-6">
      ...
      <div className="mt-4 border-t border-border"><MetricRow label="demand score" value={artist.demand} /><MetricRow label="competition score" value={artist.competition} /><MetricRow label="momentum score" value={artist.momentum} /><MetricRow label="opportunity score" value={artist.opportunity} active /></div>
      <div className="mt-5 flex flex-wrap gap-1.5">{artist.genres.map((genre) => <span key={genre} className="border border-border px-2 py-1 font-heading text-[9px] text-muted-foreground">{genre}</span>)}</div>
    </div>
  </section>
}
