...
export function TrendRow({ artist, rank, change, selected, onSelect }: TrendRowProps) {
  ...
  return <button onClick={onSelect} className={cn("group grid w-full grid-cols-[2rem_minmax(0,1fr)_auto_2.2rem] items-center gap-2 border-b border-border px-3 py-4 text-left transition-colors last:border-b-0 hover:bg-muted/40", selected && "bg-primary/[0.06]")}>
    ...
    <span className="flex min-w-0 items-center gap-2"><span className="truncate text-[13px] font-medium tracking-tight">{artist.name} <span className="text-muted-foreground">type beat</span></span>{hasLowData && <span className="shrink-0 border border-destructive/50 px-1.5 py-0.5 font-heading text-[8px] tracking-wide text-destructive">low data</span>}</span>
    ...
    <span key={artist.score} className="value-pop text-right font-heading text-xs tabular-nums">{(artist.score / 10).toFixed(1)}</span>
  </button>
}
